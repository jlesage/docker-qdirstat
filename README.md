...
This project provides a Docker container for [QDirStat](https://github.com/shundhammer/qdirstat).
...
> [!NOTE]
> This Docker container is entirely unofficial and not made by the creators of QDirStat.
...
## Table of Contents
   ...
   * [Cleaning Functionalities](#cleaning-functionalities)
...
services:
  qdirstat:
    ...
    ports:
      - "5800:5800"
    volumes:
      - "/docker/appdata/qdirstat:/config:rw"
...
[Releases]: https://github.com/jlesage/docker-qdirstat/releases
[Docker Hub]: https://hub.docker.com/r/jlesage/qdirstat/tags
...
container, it would proxy all HTTP requests for `qdirstat.domain.tld` to
...
server {
	...
	server_name qdirstat.domain.tld;

	location / {
	        proxy_pass http://docker-qdirstat;
	}
...
server {
	...
	location = /qdirstat {return 301 $scheme://$http_host/qdirstat/;}
	location /qdirstat/ {
		proxy_pass http://docker-qdirstat/;
		# Uncomment the following line if your Nginx server runs on a port that
		# differs from the one seen by external clients.
		#port_in_redirect off;
		location /qdirstat/websockify {
			proxy_pass http://docker-qdirstat/websockify;
			proxy_http_version 1.1;
			proxy_set_header Upgrade $http_upgrade;
			proxy_set_header Connection $connection_upgrade;
			proxy_read_timeout 86400;
		...
		# Needed when audio support is enabled.
		location /qdirstat/websockify-audio {
			proxy_pass http://docker-qdirstat/websockify-audio;
			proxy_http_version 1.1;
			proxy_set_header Upgrade $http_upgrade;
			proxy_set_header Connection $connection_upgrade;
			proxy_read_timeout 86400;
...
| **Application version** label | Displays the version of QDirStat integrated into Docker image. |
...
Any desktop notification generated by QDirStat is forwarded to the
...
In order to use the cleaning functionalities provided by QDirStat,
